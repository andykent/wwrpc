(function() {
  ...
  WWRPC.spawnWorker = function(protocol) {
    return new WWRPC.Worker(protocol);
  };
  ...
  WWRPC.Protocol = (function() {
    ...
    Protocol.prototype.workerCode = function() {
      var data;

      data = JSON.stringify(this.process(this.template));
      return "" + (WWRPC.bridgeCode('__bridge__')) + "\n__bridge__.unpack(" + data + ", self);\n__bridge__.init();";
    };

    Protocol.prototype.processLeaf = function(leaf, context) {
      var key, o, value;
      ...
      for (key in leaf) {
        value = leaf[key];
        o[key] = this.process(value, context.concat([key]));
      }
      return o;
    };

    Protocol.prototype.process = function(value, context) {
      if (context == null) {
        context = [];
      }
      if (value.constructor === WWRPC.RemoteFunction) {
        return value.toRpcString(context);
      }
      if (value.constructor === WWRPC.LocalFunction) {
        return value.toRpcString(context);
      }
      if (typeof value === 'object') {
        return this.processLeaf(value, context);
      }
      return value;
    };

    Protocol.prototype.call = function(name, args, callback) {
      var fn;
  ...
  WWRPC.RemoteFunction = (function() {
    ...
    RemoteFunction.prototype.run = function(args, callback) {
      if (callback !== null) {
        args.push(callback);
      }
  ...
  WWRPC.Worker = (function() {
    function Worker(protocol) {
      this.protocol = protocol;
      console.log(this.protocol);
      this.blob = new Blob([this.protocol.workerCode()], {
    ...
    Worker.prototype.process = function(data) {
      ...
      switch (data.action) {
        case 'wwrpc:call':
          return this.protocol.call(data.name, data.args, this.buildCallback(data.callbackId));
      }
    };
